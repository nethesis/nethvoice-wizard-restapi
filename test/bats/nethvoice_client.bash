...
wizard_base_url=https://127.0.0.1

# Authentication
secret=$(perl -mNethServer::Password -e "print NethServer::Password::store('nethvoice')")
mysqlDBPass=$(perl -mNethServer::Password -e "print NethServer::Password::store('mysql')")
User="admin"
fpbxPasswordHash=$(mysql asterisk -uroot -p$mysqlDBPass -B --silent -e "SELECT password_sha1 FROM ampusers WHERE username = '$User'")
...
xcurl () {
    local argv
    argv=("${@}")
    argv[$((${#argv[@]} - 1))]="${wizard_base_url}${argv[-1]}"
    curl -k \
      -s -i \
      -H 'Accept: application/json, application/problem+json' \
      -H "User: ${User}" -H "SecretKey: ${SecretKey}" \
      -X "${argv[@]}"
}
